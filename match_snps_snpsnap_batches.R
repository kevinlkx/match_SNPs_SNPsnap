...
option_list = list(
  ...
  make_option("--thresh_dist2gene", action="store", default=0.5, type='double',
              help="Distance to nearest gene [optional]"),
  make_option("--thresh_gene_density", action="store", default=0.5, type='double',
              help="Gene Density [optional]"),
...
thresh_dist2gene <- opt$thresh_dist2gene
thresh_ld_friends <- opt$thresh_ld_friends
thresh_gene_density <- opt$thresh_gene_density
...
get_matching_SNPs <- function(snpsnap_input, snpsnap_db, exclude_HLA = TRUE, thresh_MAF, thresh_dist2peak, thresh_dist2gene, thresh_ld_friends, thresh_gene_density){
  maf_range <- c(snpsnap_input$snp_maf - thresh_MAF, snpsnap_input$snp_maf + thresh_MAF)
  dist2gene_range <- snpsnap_input$dist_nearest_gene * c(1-thresh_dist2gene, 1+thresh_dist2gene)
  ld_friends_range <- snpsnap_input$friends_ld05 * c(1-thresh_ld_friends, 1+thresh_ld_friends)
  gene_density_range <- snpsnap_input$gene_count * c(1-thresh_gene_density, 1+thresh_gene_density)

  ## meet SNP Matching Criteria or not
  match_maf <- with(snpsnap_db, snp_maf >= maf_range[1] & snp_maf <= maf_range[2])
  match_dist2gene <- with(snpsnap_db, dist_nearest_gene >= dist2gene_range[1] & dist_nearest_gene <= dist2gene_range[2])
  match_ld <- with(snpsnap_db, friends_ld05 >= ld_friends_range[1] & friends_ld05 <= ld_friends_range[2])
  match_gene_density <- with(snpsnap_db, gene_count >= gene_density_range[1] & gene_count <= gene_density_range[2])

  ## get matched indices
  idx_matched <-  which(match_maf & match_ld & match_dist2gene & match_gene_density)
...
match_snps_db <- function(snpsnap_input, snpID_list_input_exist, exclude_HLA = TRUE){
  ...
  k = 1
  while(k < 6){
    snpID_matched <- get_matching_SNPs(snpsnap_input, snpsnap_db, exclude_HLA, thresh_MAF_array[k], thresh_dist2peak_array[k], thresh_dist2gene_array[k], thresh_ld_friends_array[k], thresh_gene_density_array[k])
...
thresh_dist2gene_array <- seq(thresh_dist2gene/5, thresh_dist2gene, length.out = 5)
thresh_ld_friends_array <- seq(thresh_ld_friends/5, thresh_ld_friends, length.out = 5)
thresh_gene_density_array <- seq(thresh_gene_density/5, thresh_gene_density, length.out = 5)
